    ...
    for ep_num in trange(num_episodes):
        for step in range(args.max_episode_length):
            ...
            if total_num_steps % args.log_interval == 0:
                ...
                print(log)
                logging.info(log)
                #writer.add_scalar('local loss/loss', np.mean(l_action_losses), total_num_steps)
                ...
                # writer.add_scalar('SLAM loss/proj', np.mean(costs), total_num_steps)
                # writer.add_scalar('SLAM loss/exp', np.mean(exp_costs), total_num_steps)
                # writer.add_scalar('SLAM loss/pose', np.mean(pose_costs), total_num_steps)
            # ------------------------------------------------------------------
